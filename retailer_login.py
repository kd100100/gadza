...
def get_all_farmer_products(cursor):
...
def delete_entry_retailer(item,cursor,con):
...
def get_my_requests(Name,cursor):
...
def add_entry_retailer(Name,data,cursor,con):
    if(data['IsOrganic']=="1"):
        isOrganic = 1
    else:
        isOrganic = 0
    ...
    cursor.execute("""INSERT INTO `{}` (`GoodsName`, `IsOrganic`, `DatePosted`, `Quantity`, `Region`, `OfferedPrice`, `Shipping`, `IsAvaliable`) VALUES ('{}','{}','{}','{}','{}','{}','{}','1')"""
    .format(Name,data['product'],isOrganic,datE,data['Quantity'],data['Region'],data['Price'],sHip))
    con.commit()